...
parser.add_argument('--task_name', type=str, default='union_train_turbo_loramask_rank4_continue6')
...
parser.add_argument('--pcc', action='store_true', default=True)
...
parser.add_argument('--accumulation_steps', type=int, default=18)
...
class BaseTrainer:
    ...
    def set_optimizer(self):
        if self.args.multi_gpu:
            sam_model = self.model.module.sam
        else:
            sam_model = self.model.sam

        # 设置参数组
        param_groups = []

        # image_encoder 参数 (不包括 LoRA 部分)
        image_encoder_params = [p for n, p in sam_model.image_encoder.named_parameters() if p.requires_grad]
        if image_encoder_params:
            param_groups.append({'params': image_encoder_params, 'lr': self.args.lr * 0.1})

        # prompt_encoder 参数
        prompt_encoder_params = [p for p in sam_model.prompt_encoder.parameters() if p.requires_grad]
        if prompt_encoder_params:
            param_groups.append({'params': prompt_encoder_params, 'lr': self.args.lr * 0.01})

        # mask_decoder 参数 (不包括 LoRA 部分)
        mask_decoder_params = [p for n, p in sam_model.mask_decoder.named_parameters() if p.requires_grad]
    ...
    def save_checkpoint(self, epoch, state_dict, describe="last"):
        ...
        # 保存LoRA参数
        if self.args.multi_gpu:
            self.model.module.save_lora_parameters(join(MODEL_SAVE_PATH, f"lora_params_{describe}.pth"))
        else:
            self.model.save_lora_parameters(join(MODEL_SAVE_PATH, f"lora_params_{describe}.pth"))
