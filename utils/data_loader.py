...
class Dataset_Union_ALL(Dataset): 
    ...
    def __getitem__(self, index):
        ...
        subject = tio.Subject(
            image=tio.ScalarImage(tensor=sitk_image_arr.astype(np.float32)),
            label=tio.LabelMap(tensor=sitk_label_arr.astype(np.uint8)),
        )
        ...
        subject = tio.CropOrPad(mask_name="label", target_shape=(self.image_size, self.image_size, self.image_size))(subject)
